...
## Save/Request Review moved up under the Approved checkbox (2026-09-10)

Reported directly: "can we move the save and request review buttons up to directly underneath the Approve checkbox so scrolling down to them is not required. leave the notes space underneath those buttons to be used if needed." Straightforward reorder inside `renderDeskPanelBody`'s notice-panel block: the code that builds `saveRow` (Save + Request Review) and `reviewHint` now runs immediately after the Approved checkbox and its `approval-meta` status line, and the `notesLabel`/`notesArea` pair that used to sit between them now comes last instead — DOM order is Status dropdown → Approved checkbox → status line → Save/Request Review → hint text → Notes. Only reorder: nothing about what Save or Request Review actually do, or what gets saved, changed.

One wording fix the move required: the Request Review hint used to read "...raised in the notes above..." — true when Notes sat above the buttons, wrong now that it's below. Changed to "notes below." Verified via Playwright: read the `.approval-block`'s children in DOM order to confirm the new sequence directly (not just visually), confirmed Save still saves the checkbox state and preserves existing notes correctly from its new position, and checked the hint text in both its "Request Review" and "Cancel Review Request" states. See `CHANGELOG.md` for the tracked record.

## A "back" button from a desk window to the list it was opened from (2026-09-10)

Reported directly: "I do not want to change the notice board, and bookshelf, or file cabinet from the opaque backgrounds and green boarders they are on now, but we need a way to return to them if a document etc is opened and the user wants to open another from there." — i.e. leave the Notice Board/File Cabinet/Bookshelf overlay panels exactly as they look today, but stop making the × the only way out of an opened item, since × always drops straight to the plain shed view. Opening any document from one of these lists (the "Desktop" button on each row) has always closed the panel outright — that's still true and unchanged — so there was previously no way back to the list itself except re-clicking the hotspot from the shed view and, for anything filed in a nested File Cabinet folder, re-drilling all the way back down to where it was.

**The mechanism.** Every "Desktop" button click (`buildItemRow`'s `sendBtn` handler, inside `openPanel`) now passes a third argument to `addToDesk`: `{ key, path, label }` — `key` is the panel (`board`/`cabinet`/`bookshelf`/`rug`/`bin`), `label` is that panel's own title (`data.title`, e.g. "Notice Board"), and `path` is the exact File Cabinet folder the item was filed in at the moment "Desktop" was clicked (`null` for the flat Notice Board/Bookshelf lists, which have no folders to remember). `addToDesk` stores this as `win.cameFrom` on the desk window it opens (or updates on an existing one, if the same item is sent again from a different context). Every other `addToDesk` call site in the file — a notice's own "📄 Review: `<document>`" button, a document's "⚑ N approval notice(s) pending" link back to its notice, a to-do's "View Notice" link, `openTodoDesk` — passes nothing, so those windows simply have no `win.cameFrom` and get no back button; only an item actually sent from one of these three list panels remembers where it came from.

`renderDeskChrome` now renders a small ← button, in whichever chrome mode is active, whenever the window it belongs to has a `win.cameFrom`: next to the × in the single-item titlebar (`dw-titlebtns`, same round-button styling as maximize/close), and — only on the currently active tab, since that's the one actually on screen — right before that tab's own × when two or more windows are open. Clicking it (`buildBackBtn`) calls `openPanel(win.cameFrom.key, stageEl, win.cameFrom.path)`: this reopens the panel exactly as if the hotspot had been clicked fresh (title, live refetch and all — see the earlier stale-list fix above, unaffected), laid on top of the desk (the overlay's `z-index:20` sits above the desk-window stack's `z-index:15`, so it visually covers the still-open item beneath it) — and, critically, it does **not** close the item's own window. Closing the reopened panel (its own × ) just removes the overlay and reveals that window again, unchanged, exactly where it was left. This means back-and-forth between a list and an item it opened never discards either side; the only way to actually close an opened item is still its own ×, exactly as before.

**Restoring the exact File Cabinet folder** needed one more piece, since `openPanel`'s cabinet branch always started at the folder-grid root (`renderFolders()`) before today. `openPanel` now takes an optional third parameter, `initialPath`; when set, it splits that path into its segments, builds the same chain of nested-folder calls a real click-by-click drill-down would produce (`renderCabinetLevel` at each level, each one's own "← Back" wired to its actual parent — the last one back to the folder grid), and jumps straight to the deepest level. A new `currentCabinetPath` variable, set at the top of `renderCabinetLevel` every time it runs, is what `buildItemRow`'s Desktop handler reads to know which folder an item is actually sitting in the moment it's sent to the desk — this is also the mechanism that already made the in-panel "Filed directly in this folder:" list correctly folder-scoped, just newly reused here. The upshot: send a document to the desk from three folders deep, open something else, come back — back reopens directly at that same three-folders-deep view, with its own "← Back" still correctly pointed at its real parent folder, not the root.

Verified via Playwright: a notice sent from the Notice Board reopens the Notice Board (title, overlay visibly open) via back, with the notice's own window confirmed still present and showing its title once that reopened panel is closed again; a document sent from `Knowledge Curation System/Mother Information Library/ARCs` (three folders deep) reopens at that exact path, matching the title captured right before "Desktop" was clicked, byte for byte; the To-Do List (never opened from a list) shows no back button; and in tabbed mode, only the active tab's pill carries one — an inactive tab (the To-Do List, sitting behind the just-opened book) has none. See `CHANGELOG.md` for the tracked record.

## Notice status/approval changes now reach an already-open To-Do List tab (2026-09-10)

Reported directly: "I just changed a notice status to approved and in progress and saved and nothing changed in the to-do-list — it still shows as new not in progress. Also nothing shows in the to-do-list saying it has been approved." Both true, and both the same root cause. The notice panel and the To-Do List are each their own desk-window tab (see the tab-strip rework and the pseudo-item pattern above) — completely separate closures, each with its own local copy of the linked to-do row and the linked notice's approval state. Changing the status dropdown or the Approved checkbox in the notice panel updates that panel's own DOM and its own copy of `item`/`CONTENT`, and nothing more; an already-open To-Do List tab had no way to find out anything had changed, so it kept showing whatever was true when it last loaded, exactly like the earlier stale-list bug this session — just one level down, between two tabs instead of between a save and a reopen.

Fixed with one new helper, `refreshRelatedDeskWindows(stageEl, {todoId, skip})`, called after every RPC that changes a to-do's status (`shed_set_todo_status`) or a notice's approval/review state (`shed_set_notice_approval`, `shed_request_notice_review`, `shed_clear_notice_review`) succeeds — from both places that can trigger one: the notice panel's own Status dropdown and Approved-checkbox/Request-Review controls, and the To-Do List row's own status dropdown. It walks every currently-open desk window and, for the To-Do List tab (if open — it shows every to-do, not just one, so it's always refreshed) and for any notice panel open on the matching `todoId`, tears down and rebuilds that window's body via the existing `refreshDeskWindow` (the same function a saved title/body edit already used) — `skip` leaves out the window the change was just made in, since that one already updated itself in place. The practical effect: change a notice's status or approval from its own panel, switch to the still-open To-Do List tab, and it's already showing the new status and decision — no reload, no reopening the tab.

Same request, a second piece: the To-Do List's own rows had no way to show a linked notice's actual decision at all — only the shared workflow status (New/Received/.../Completed), never whether the notice itself had been approved. Each row now carries the same `.item-row-decision` chip already shown on the Notice Board list row (✓ Approved / ⚑ Review Requested / Not Yet Approved), computed from the same `linkedNotice` lookup the row already used for its "View Notice" link and Completed-disabling check, so a plain manually-added to-do with no linked notice correctly shows no badge at all.

Verified via Playwright with a stubbed `shed_set_todo_status`/`shed_set_notice_approval` that actually mutate in-memory server state (not just static fixtures): opened the To-Do List, then the linked notice as a second tab from its "View Notice" link; changed the notice panel's Status to In Progress and checked Approved; switched back to the still-open To-Do List tab and confirmed its row's status select read "In Progress" and its decision badge read "✓ Approved" with no reload. Then the reverse: changed the status from the To-Do List row itself, switched to the still-open notice panel, and confirmed its Status dropdown had picked up the change too. A separate render check (three to-dos: approved-linked, review-requested-linked, and a plain unlinked task) confirmed the badge only appears where a notice is actually linked, and reads the right colour/text in the other two states. See `CHANGELOG.md` for the tracked record.
