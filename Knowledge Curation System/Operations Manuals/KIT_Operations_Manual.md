...
**Version 0.3 addition:** §10.5's PKR identifier format is now Founder-confirmed rather than provisional, and a new §10.2A confirms a subject-scope extension for non-plant-specific records (`EVIDENCE-CONFIDENCE` and similar), both per Founder decision 23 August 2026 clearing a batch of pending items from the project backlog.

**Version 0.2 addition:** Chapter 7 gains §7.7, a Wording Boundary implementing PKR Standard §4.3 (Draft v0.07): KIT may add gardener-friendly wording when drafting PKR content but shall never let that wording alter, extend or reinterpret the underlying documented claim. Added at Founder direction alongside the ruling on how ROC's own reasoned synthesis is handled (Evidence Assessment Standard v1.4 §2.10) — the Founder identified this as the same concern one layer downstream, where KIT converts Assessed Findings into gardener-facing content.
...
## 7.7 Wording Boundary — Clarify, Never Alter

Every field built under this chapter requires KIT to convert an Assessed Finding's or Founder observation's documented wording into content a beginner gardener can use — for example, a Suitability Gate PKR's Question or Check (PKR Standard §5.4), or an Observation PKR's Visual Criteria descriptions (§7.3). Per PKR Standard §4.3, this translation may improve clarity, tone, formatting or structure, but shall never change what the documented claim asserts, extend it beyond what the evidence covers, add an explanatory "why" the source material did not itself state, or soften, omit or fold in a preserved limitation.

Where making a claim gardener-readable requires resolving a genuine ambiguity in the claim itself — not merely choosing plainer words for the same meaning — KIT shall treat this as a gap under Chapter 9 and refer it to the Founders, rather than resolve the ambiguity through its own inference. This is the same discipline Chapter 2.3 ("Flag, Never Fill") already requires for missing content, applied here to wording that risks silently adding meaning rather than omitting it.
...
## 10.2 Identifier Format

**`PKR-<TYPE-CODE>-<sequence>`**
...
**This format deliberately carries no subject, plant, or scope information.** An earlier version of this format included a `<SUBJECT-SCOPE>` segment (for example, `BUSHROSE-DEADWOOD`), mirroring the FRD/ARC convention. That was reconsidered and dropped, at Founder direction, 23 August 2026, for three reasons: the information is already fully captured, twice over, in two required Common Fields every PKR already carries — Title and Applies To — so the ID segment was pure duplication, with the same drift risk this project has corrected at every other layer (two places holding the same fact, able to silently disagree); a record's real scope can change over time (Applies To can be revised), while §10.3 requires the identifier never to change, so a scope segment baked into an ID was never reliably accurate for long; and Pip's retrieval should query structured fields (PKR Type, Applies To, and similar), never parse meaning out of an ID string — an opaque, fixed-width key is faster and simpler for that purpose in any datastore. This reasoning surfaced while designing Source PKRs specifically (where the reuse problem was most obvious — the same citation backing multiple, unrelated findings) but applies equally to every PKR type, so the whole convention was corrected at once rather than leaving Source PKRs as a special case.

## 10.2A Human Readability Without Scope in the ID

Since the ID itself is now opaque, two other things carry the human-facing description KIT still needs for review and discussion:

- **Title** (PKR Standard §4) is the primary human-facing descriptor and should be written descriptively enough to identify the record without needing its ID.
- **Filenames are not identifiers.** A working file may still use a descriptive name (for example, `PKR-OBS-BUSHROSE-DEADWOOD-01-submission.md`) for ease of navigation during drafting and review — this is a filesystem convenience, decoupled from the record's canonical ID, and does not need to change when the ID does or stay in sync with it.

## 10.2B Correcting an Identifier Before Publication

§10.3 requires a PKR's identifier to be permanent once assigned. That rule exists to protect anything that has come to depend on the identifier — a published record, a cross-reference from another published record, or (eventually) a live gardener-facing session. Before a record has reached **Approved for Publication**, nothing outside the Knowledge Curation System depends on its ID yet, so an identifier may still be corrected under explicit Founder direction — exactly what happened 23 August 2026, when the eight then-existing Draft records (all still short of Approved for Publication, per Chapter 11) were migrated from the retired `<SUBJECT-SCOPE>` format to this one.

*Correction, 24 August 2026: this section previously read "four... records," miscounting the four PKR* types *involved (Observation, Decision Logic, Suitability Gate, Definition) as four individual records. The table at §10.5 always correctly listed all eight — this was a prose-only error, now corrected to match it.* This is the only circumstance in which KIT changes an already-assigned identifier; once a record reaches Approved for Publication, §10.3's permanence applies without exception.
...
## 10.5 Founder Confirmation

Per PKR Standard §6, this format required Founder confirmation before binding use. The subject-scoped version of this format was confirmed 23 August 2026, then reconsidered the same day and replaced by the scope-free format at §10.2, per the reasoning recorded there — the current, controlling confirmation. The eight Draft PKRs that existed under the retired format were migrated to it, per §10.2B:

| Retired ID | Current ID |
|---|---|
| PKR-OBS-BUSHROSE-DEADWOOD-01 | PKR-OBS-000001 |
| PKR-DEC-BUSHROSE-DEADWOOD-01 | PKR-DEC-000001 |
| PKR-SGT-BUSHROSE-DORMANCY-01 | PKR-SGT-000001 |
| PKR-DEF-EVIDENCE-CONFIDENCE-01 | PKR-DEF-000001 |
| PKR-DEF-EVIDENCE-CONFIDENCE-02 | PKR-DEF-000002 |
| PKR-DEF-EVIDENCE-CONFIDENCE-03 | PKR-DEF-000003 |
| PKR-DEF-EVIDENCE-CONFIDENCE-04 | PKR-DEF-000004 |
| PKR-DEF-EVIDENCE-CONFIDENCE-05 | PKR-DEF-000005 |
...
- every field's wording says the same thing as the documented claim it traces to, no more and no less, per Chapter 7.7;
